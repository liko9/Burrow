...
// KafkaAdminClient is a consumer module which connects to a single Apache Kafka cluster and reads consumer group information
// and their offsets from the cluster, using the AdminClient API. The offset details for consumer groups
// is forwarded to the storage subsystem for use in evaluations.
...
// consumers belong, as well as a list of servers provided for the Kafka cluster, of the form host:port.
// If the cluster name is unknown, or if the server list is missing or invalid, this func will panic.
func (module *KafkaAdminClient) Configure(name string, configRoot string) {
...
// Start connection to the Kafka cluster using the Shopify/sarama client. Any error connecting to the cluster is returned
// to the caller. Once the client is set up, the consumer groups and their offsets are gathered.
func (module *KafkaAdminClient) Start() error {
	module.Log.Info("starting")

	// Connect Kafka AdminClient
	client, err := sarama.NewClusterAdmin(module.servers, module.saramaConfig)
	if err != nil {
	...
	// Start the consumer
	err = module.startKafkaAdminClient(&helpers.BurrowSaramaClient{Client: client})
	if err != nil {
...
// Stop closes the goroutines that listen to the AdminClient.
func (module *KafkaAdminClient) Stop() error {
...
func (module *KafkaAdminClient) partitionConsumer(consumer sarama.PartitionConsumer, stopAtOffset *backfillEndOffset) {
...
func (module *KafkaAdminClient) startKafkaAdminClient(client helpers.SaramaClient) error {
	// Create the consumer from the client
	consumer, err := client.NewClusterAdminFromClient()
	if err != nil {
		module.Log.Error("failed to get new AdminClient", zap.Error(err))
		client.Close()
		return err
	}
...
func (module *KafkaAdminClient) processConsumerOffsetsMessage(msg *sarama.ConsumerMessage) {
...
func (module *KafkaAdminClient) decodeAndSendOffset(offsetOrder int64, offsetKey offsetKey, valueBuffer *bytes.Buffer, logger *zap.Logger, decoder func(*bytes.Buffer) (offsetValue, string)) {
...
func (module *KafkaAdminClient) decodeGroupMetadata(keyBuffer *bytes.Buffer, value []byte, logger *zap.Logger) {
...
func (module *KafkaAdminClient) decodeAndSendGroupMetadata(valueVersion int16, group string, valueBuffer *bytes.Buffer, logger *zap.Logger) {
